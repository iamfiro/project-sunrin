...
const NoteLine = ({ lane, notes, scroll, noteDisplayTime, pressedKeys }: NoteLineProps) => {
    const visibleNotes = notes.filter(note => {
        const noteTime = note.time;
        const isVisible = noteTime >= scroll && noteTime <= scroll + noteDisplayTime;
        return isVisible;
    });
    ...
    return (
        <div className={cn(s.container, { [s.keyPressed]: isKeyPressed })}> {/* Apply class conditionally */}
            {visibleNotes.map(note => {
                const position = 1 - (note.time - scroll) / noteDisplayTime;
                return <NoteComponent key={note.id} note={note} position={position} noteDisplayTime={noteDisplayTime} />
            })}
        </div>
    )
}
