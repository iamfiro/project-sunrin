...
import JudgmentLine from "../judgment-line"
...
export default function PlayField() {
    ...
    const handleKeyPress = useCallback((keyIndex: number) => {
        ...
        if (closestNote) {
            ...
            if (timeDiff <= JUDGEMENT_WINDOWS.good) {
                setNotes(notes => notes.filter(note => note.id !== closestNote!.id));
            }
        }
    }, [notes]);
    ...
    useEffect(() => {
        // Miss check
        const missedNotes = notes.filter(note => scroll > note.time + JUDGEMENT_WINDOWS.miss);
        if (missedNotes.length > 0) {
            showJudgement("Miss");
            setCombo(0);
            setNotes(notes => notes.filter(note => !missedNotes.some(missed => missed.id === note.id)));
        }
    }, [scroll]);

    return (
        <div className={s.container}>
            <GameStats score={score} combo={combo} />
            ...
            <div className={s.keyBox}>
                {keys.map((key, index) => (
                    <div 
                        key={index} 
                        className={cn(s.key, { [s.keypress]: pressedKeys.has(index) })}
                    >
                        {key}
                    </div>
                ))}
